...
function voiceBlip(ctx: BaseAudioContext, dest: AudioNode, label: string, when: number, dur: number, vol: number): void {
  const h = labelHash(label);
  const f0 = 150 + h * 90;
  const osc = ctx.createOscillator();
  osc.type = "sawtooth";
  ...
  osc.frequency.linearRampToValueAtTime(f0 * 0.94, when + dur);
  const bp = ctx.createBiquadFilter();
  bp.type = "bandpass";
  bp.frequency.value = 900 + h * 600;
  bp.Q.value = 1.1;
  const gain = ctx.createGain();
  // syllabic amplitude wobble (~4 Hz) so it reads as speech, not a beep
  gain.gain.setValueAtTime(0, when);
  const syllables = Math.max(3, Math.round(dur / 0.28));
