...
import { identityPanelData, scoreProjectIdentity, scoreShotIdentity } from "@/lib/identity";

// ── Identity-similarity scoring for panels.
//
// GET   ?projectId=                        -> scored rows (worst first) + drift queue + picker
// POST  { shotId }                         -> score ONE panel against its cast's model sheets (vision)
// PATCH { projectId, limit? }              -> batch score (worst existing scores first, then unscored)
