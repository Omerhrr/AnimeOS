...
async function main() {
  ...
  check("A5 the registry grew to 62 tools (design_motion joined in iter 52)", toolCount === 62, `count=${toolCount}`);
  ...
  check("A7 the v6 builder supports PROP + CREATURE kinds", builder.includes('kind == "PROP"') && builder.includes('kind == "CREATURE"') && builder.includes("ASSET BUILDER (v6.0)"));
  ...
  check("D6 the rebuild under designed recipes records the pairing (v2, v6 builder)", buildRigged.status === "OK" && propAsset2?.version === 2 && meta2?.materialRecipe?.name === "E2E Spirit Steel" && meta2?.lightingRig?.name === "E2E Moonlit" && meta2?.builderVersion === "v6.0", buildRigged.result.slice(0, 140));
  ...
  check("E4 the audit names real, concrete issues (severity + kind)", plainIssues.length > 0 && [...kinds].every((k) => ["GEOMETRY", "MATERIAL", "SILHOUETTE", "PROPORTION", "PALETTE", "LIGHTING", "DETAIL", "IDENTITY", "MOTION"].includes(k)), `issues=${plainIssues.length} kinds=${[...kinds].join(",")}`);
