...
- **img2vid provider slot.** Set `ANIMEOS_IMG2VID_HOST` to any service speaking the tiny jobs protocol (POST `/jobs` with the key art + pose pair, GET `/jobs/{id}` until `done` + `videoUrl`) and pose-carrying hero shots route to the interpolation model before the built-in engines; the finished clip downloads back into `public/renders/`. Unset (the default), the slot is invisible.
...
1. **Performance-grade character motion.** Characters now move inside the frame as a blocking pass: shots carry start/end pose chips (13-pose vocabulary), the Blender worker articulates a skeletal stand-in between them, an img2vid provider slot renders the key art between poses when one is attached, and the MOTION engine plays an impact-beat approximation. What is still missing: learned interpolation quality (real img2vid models vs stand-in blocking), facial performance and finger-level articulation, and physics-accurate interaction with props and cloth. Next: wire a real img2vid provider behind the existing `ANIMEOS_IMG2VID_HOST` slot, add per-state pose presets so a character's development state drives HOW a pose is performed, and upgrade the stand-in rig (faces, hands, secondary motion).
