...
- **Headless Blender (auto-detected).** If a `blender` binary is on the machine (or `ANIMEOS_BLENDER_BIN` points at one), each render job spawns a fresh worker: `blender -b -P bridges/blender/animeos_bridge.py -- --worker --job <file>`. The worker builds a stand-in 3D scene (rocky terrace, emissive blade, moon/rim/energy lights), drives a camera rig through the shot's movement grammar frame by frame, applies the AnimeOS scene parameters, renders with Cycles CPU and encodes h264 (system ffmpeg, or Blender's own FFMPEG writer). One 3D worker at a time; overflow jobs fall to the MOTION engine.
- **MOTION engine (built-in).** A deterministic camera program (per-movement zoom/pan/tilt profiles x shot-type framing x lens character x lighting grade x fog/lightning/energy) is animated over the shot's key art with ffmpeg zoompan - or over a procedural gradient plate when no art exists - and encoded at the project's fps.
...
1. Open **Render Queue**, trigger a render on any shot - the **Engine driver** card shows whether headless Blender workers or the built-in MOTION engine is driving; the job badge shows `BLENDER` or `MOTION`. Watch progress stream per frame, then **play the finished clip right on the job card** - a dolly-in on a closeup dollies, a pan pans.
...
1. **Character motion inside the frame.** Shots now move as real clips, but the motion is camera grammar: pans, dollies and cranes over key art (MOTION) or a 3D stand-in scene (Blender). Characters do not yet walk, fight or emote frame to frame. Next: an img2vid pass on hero shots (per-shot keyframe pairs + an interpolation model slot behind the same driver chain), Blender-side skeletal stand-ins for blocking passes, and shot-level motion chips (start/end poses) the same way movement chips exist today.
